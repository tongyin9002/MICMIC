...
PC_para<-function(data_matrix,max_L=1,method=c("CMII","CMI"),pre_adj=NULL,log_file_dir=NA,edgemode=c("pvalue"),pvalue_cut=0.01,core_num=1,permutation_times=100)
{
 ...
 ###sTART
   ...
   #start L runs of PC
    ...
    {   gc()  #clear memory at each time
        ...
        if(L==0)
        {
         result<-PC_one_run_para_L0(data_matrix,mynet@edges,L,method=method,log_file_dir=log_file_dir,pvalue_cut=pvalue_cut,core_num=core_num,permutation_times=permutation_times)
        }
        ...
        if(sum(mynet@adj_matrix)>0)
        {
         if(L==1)
         {
          result<-PC_one_run_para_L1(data_matrix,mynet,L,method=method,log_file_dir=log_file_dir,pvalue_cut=pvalue_cut,core_num=core_num,permutation_times=permutation_times)
         }
        }else
        {
          result<-data.frame()
...
PC_one_run_para_L0<-function(data_matrix,L0_edges,L,method=c("CMII","CMI"),log_file_dir=NA,edgemode=c("pvalue"),pvalue_cut=0.01,core_num=1,permutation_times=100)
{   options(stringsAsFactors = FALSE)
    method <- match.arg(method)
    edgemode <- match.arg(edgemode)
 ...
 ###build a result data frame to store the MI value and pvalue for each edge
        ...
        L0_result<-cbind(nodenames[L0_edges[,1]],nodenames[L0_edges[,2]],rep(NA,nrow(L0_edges)),as.numeric(result_para[,1]),as.numeric(result_para[,2]),as.numeric(result_para[,3]))
        colnames(L0_result)<-c("NodeA","NodeB","condition","nor_MI_value","median_of_test","adj.pvalue")
        L0_result2<-cbind(nodenames[L0_edges[,2]],nodenames[L0_edges[,1]],rep(NA,nrow(L0_edges)),as.numeric(result_para[,1]),as.numeric(result_para[,2]),as.numeric(result_para[,3]))
        colnames(L0_result2)<-c("NodeA","NodeB","condition","nor_MI_value","median_of_test","adj.pvalue")
...
PC_one_run_para_L1<-function(data_matrix,mynet,L,method=c("CMII","CMI"),log_file_dir=NA,edgemode=c("pvalue"),pvalue_cut=0.01,core_num=1,permutation_times=100)
{ options(stringsAsFactors = FALSE)
  method <- match.arg(method)
  edgemode <- match.arg(edgemode)
  ...
  if(nrow(L1_edgesinCondition)>0)
  {
    ...
    colnames(L1_result)<-c("NodeA","NodeB","condition","nor_CMI_value","median_of_test","adj.pvalue")
    L1_result2<-cbind(L1_result[,2],L1_result[,1],L1_result[,c(3:6)])
    colnames(L1_result2)<-c("NodeA","NodeB","condition","nor_CMI_value","median_of_test","adj.pvalue")
...
callmcL0test<-function(L0_edges,data_matrix,core_num=1,permutation_times=30)
{
    ...
    return(result[,c("nor_MI_value","median_of_test","adj.pvalue")])
}
