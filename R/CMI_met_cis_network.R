...
CMI_met_cis_network<-function(met_data_matrix,exp_data_matrix,gene_list,distance=300000,ref_gene_bed,ref_CpGs_bed,outfiledir=NA,pvalue_cut=0.001,core_num=1,permutation_times=100,MI_cut=0.1)
{
...
select_associated_sites<-function(elements,ref_CpGs_bed,data_matrix,outfiledir,core_num=1)
{
    ...
    {   #cat("select associated sites ");cat(i);cat(" in ");cat(nrow(elements));cat(" ...\n")
        ...
        {
         pvalues<-unlist(mclapply(seq_len(length(cis_CpGs)),function(x){cor.test(data_matrix[gene_name,],data_matrix[cis_CpGs[x],])$p.value},mc.cores = core_num))
