...
MICMIC_plotting<-function(gene_name,met_data_matrix,exp_data_matrix,control_id=NA,distance=NA,ref_gene_bed,ref_CpGs_bed,sample_class,outfiledir=NA)
{
    ...
    network_plot<-plot_network_track(gene_list,CpGs_list,MI_net,CMI_net,gene_name,target_gene_chr,target_gene_TSS,target_gene_length,target_gene_strand,range,gene_bed,CpGs_bed,outfiledir)
    ...
    if(length(cor_direct_plot_list)>0)
    {
...
plot_genome_track<-function(gene_name,target_gene_chr,target_gene_TSS,target_gene_length,target_gene_strand,range,gene_bed,outfiledir)
{
    ...
    my_genome_plot<-my_genome_plot+
        annotate("text",x=(plot_data["||","plot_coords"]+plot_data["|","plot_coords"])/2,y=-0.3,label="promoter",size=18)
    ...
    {
        my_genome_plot<-my_genome_plot+
            annotate("text",x=mean(c(plot_data["|","plot_coords"],plot_data["TSS","plot_coords"])),y=-0.1,label="2000bp",size=14)
        my_genome_plot<-my_genome_plot+
            annotate("text",x=mean(c(plot_data["TSS","plot_coords"],plot_data["||","plot_coords"])),y=-0.1,label="500bp",size=14)
    }else
    {
        my_genome_plot<-my_genome_plot+
            annotate("text",x=mean(c(plot_data["|","plot_coords"],plot_data["TSS","plot_coords"])),y=-0.1,label="500bp",size=14)
        my_genome_plot<-my_genome_plot+
            annotate("text",x=mean(c(plot_data["TSS","plot_coords"],plot_data["||","plot_coords"])),y=-0.1,label="2000bp",size=14)
    }
...
plot_CpGs_track<-function(regulator_CpGs_names,CpGs_type,gene_name,target_gene_chr,target_gene_TSS,target_gene_length,target_gene_strand,range,ref_CpGs_bed,outfiledir)
{
    ...
    h[which(CpGs_type=="indirect")]<-0.2
    ...
    my_CpGs_track<-ggplot(plot_data,aes(x=plot_coords,y=h,colour=CpGs_type))+
        ...
        scale_colour_manual(values = c("direct"="forestgreen","indirect"="gray60"))+
...
plot_network_track<-function(gene_names,CpGs_names,MI_net,CMI_net,gene_name,target_gene_chr,target_gene_TSS,target_gene_length,target_gene_strand,range,ref_gene_bed,ref_CpGs_bed,outfiledir)
{
    plot_range<-genome_coord_to_plot_coord(c(range[1],range[2]),target_gene_chr,target_gene_TSS,target_gene_length,target_gene_strand,range)
    ...
    network_track<-network_track+
        geom_curve(aes(x=x1,xend=x2,y=0,yend=0),data=network_plot_data,curvature=-0.2,color="gray60")
    ...
    colnames(network_plot_data)<-c("x1","x2")
    network_track<-network_track+
        geom_curve(aes(x=x1,xend=x2,y=0,yend=0),data=network_plot_data,curvature=-0.2,color="lightgreen")
