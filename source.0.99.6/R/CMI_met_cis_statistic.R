...
#' @param promoter_upstrm a distance number, define the promoter region that in upstream to TSS
#' @param promoter_dwstrm a distance number, define the promoter region that in downstream to TSS
...
generate_regulator_info<-function(met_data_matrix,exp_data_matrix,gene_list,outfiledir=NA,ref_gene_bed,ref_CpGs_bed,promoter_upstrm=2000,promoter_dwstrm=2000)
{
    ...
    for(i in seq_len(length(gene_list)))
    {
        ...
        if(file.exists(paste(outfiledir,"/",gene_list[i],"_direct_regulation.txt",sep="")))
        {
         ...
         if(file.exists(paste0(outfiledir,"/",gene_list[i],"_indirect_regulation.txt")))
         {indirect_regulation<-read.table(paste(outfiledir,"/",gene_list[i],"_indirect_regulation.txt",sep=""),sep="\t")
         }else
         {indirect_regulation<-data.frame("","","","");indirect_regulation<-indirect_regulation[-1,]
         }
         ...
         mutual_information<-c(rep(NA,length(association_sites)),direct_regulation[,3],rep(NA,nrow(indirect_regulation)))
         ...
         regulator_type<-c(rep("association",length(association_sites)),rep(direct_regulation[,4]),rep("indirect",nrow(indirect_regulation)))
         ...
         regulator_end[which(elements_type=="CpGs_met")]<-regulator_start[which(elements_type=="CpGs_met")]
         ...
         if(ref_gene_bed[gene_list[i],"strand"]=="+")
         {
            proximal_start<-ref_gene_bed[gene_list[i],"start"]-promoter_upstrm
            proximal_end<-ref_gene_bed[gene_list[i],"end"]
         }
         if(ref_gene_bed[gene_list[i],"strand"]=="-")
         {
            proximal_start<-ref_gene_bed[gene_list[i],"start"]
            proximal_end<-ref_gene_bed[gene_list[i],"end"]+promoter_upstrm
         }
         ...
         if(ref_gene_bed[gene_list[i],"strand"]=="+")
         {
            regulation_dis[which((regulation_dis=="promoter")&(regulation_distance>promoter_dwstrm))]="gene_body"
         }
         if(ref_gene_bed[gene_list[i],"strand"]=="-")
         {
            regulation_dis[which((regulation_dis=="promoter")&(regulation_distance<(-promoter_dwstrm)))]="gene_body"
         ...
         if(file.exists(paste0(outfiledir,"/",gene_list[i],"_cor_test.txt")))
         {cor_result<-read.table(paste0(outfiledir,"/",gene_list[i],"_cor_test.txt"),sep="\t")
          rownames(cor_result)<-paste(cor_result[,1],cor_result[,2])
          pearson_cor<-cor_result[paste(gene_list[i],regulator_name),"cor"]
          pvalue<-cor_result[paste(gene_list[i],regulator_name),"pvalue"]
         }else
         {pearson_cor<-vector()
          pvalue<-vector()
         ...
         regulator_information<-data.frame(target_name,target_chr,target_strand,target_TSS,target_gene_length,regulator_name,regulator_type,elements_type,regulator_chr,regulator_start,regulator_end,regulation_dis,regulation_distance,met_sd,pearson_cor,pvalue,indirect_passenger,mutual_information)
...
merge_regulator_info<-function(gene_list,outfiledir=NA,statisticfiledir=NA,ref_gene_bed,distalfilter=TRUE)
{
    options(stringsAsFactors = FALSE)
    ...
    promoter_end<-TSSpos+2000
    promoter_start[minus_strand_lines]<-TSSpos[minus_strand_lines]-2000
